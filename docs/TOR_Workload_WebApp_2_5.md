...
| `object_repairs` INSERT / UPDATE | Mark this object's `summaries.is_stale = TRUE` |
...
The PoC (Proof of Concept) has one goal: **prove that the web application correctly replicates the Excel workbook's ИТОГО Числ calculation for all 2,935 objects**, verifiable by comparing СВОД output against the source file. It is the minimum build that gives stakeholders confidence to proceed to full MVP development.

PoC is not a stripped-down MVP. It is a **throwaway validator** — some of its simplifications will be reversed in MVP. The schema and codebase do not need to be production-ready.
...
| Feature | Notes |
|---|---|
| **XLSX import** | Import all 2,935 objects from source file. Equipment, repairs, records, travel. |
| **Calculation engine** | Full calculation: ОС, ПС, Видео, Записи, Ремонт, Дорога → ИТОГО Числ. All formulas from §6. |
| **СВОД table** | Paginated table matching all 19 source columns. Correct values per object. |
| **СВОД export to XLSX** | Export matching original template structure. Stakeholders verify against source file. |
| **Object detail page** | View and edit equipment quantities, repairs, records, travel for any object. |
| **Basic dashboard** | Total ИТОГО Числ per division, top objects by workload, object count. |
...
#### PoC Simplifications (intentional deviations from full TOR)

These are deliberate cuts for PoC only. Each one is reversed in MVP.

**S-01: Flat equipment model — no physical inventory layer**

Full TOR has two layers: `object_devices` (physical qty) + `object_system_assignments` (maintained qty).

PoC has one table: `equipment_entries` with columns `object_id`, `device_type_id`, `system_type`, `quantity`. This is equivalent to `object_system_assignments` only, with `quantity_physical = quantity_maintained` assumed.

*Reversed in MVP:* Add `object_devices` table; split UI into two sections.

**S-02: Synchronous recalculation on save — no staleness tracking**

Full TOR has `is_stale` flag + on-demand background job.

PoC recalculates the object summary **synchronously** when any of its data is saved (equipment, repairs, records, travel). Single-object recalculation takes ~5ms — acceptable for PoC. No background worker, no `is_stale` column.

*Reversed in MVP:* Add `is_stale` flag, background worker queue, on-demand trigger.

**S-03: Static normatives — no admin UI**

Full TOR has admin-editable device catalog and repair type catalog.

PoC loads normatives as migration seed data (all 35 device contexts + 25 repair types from source XLSX). No UI to add, edit, or delete device types, system contexts, or repair types.

*Reversed in MVP:* Add device catalog management UI (§7.8), repair type catalog UI, context restriction enforcement.

**S-04: Single role — no RBAC**

Full TOR has four roles: admin, editor, engineer, viewer.

PoC has one role: authenticated user (can do everything). Login/logout only.

*Reversed in MVP:* Implement admin / editor / engineer / viewer roles with division scoping.

**S-05: No planning periods**

Full TOR has `periods` table; repairs and records are period-scoped.

PoC stores one `object_repairs` row per (object, repair_type) and one `records_tasks` row per object — no period FK. Import assigns all data to an implicit current state.

*Reversed in MVP:* Add `periods` table, `period_id` FKs, period activation, period lock, period selector in UI.

**S-06: No engineer module**

Full TOR has engineers as first-class entities with workload splitting, capacity tracking, dashboards, and coverage gap reporting.

PoC stores `responsible_engineer` as a plain `VARCHAR(255)` on the `objects` table — the same as the source Excel. No `object_engineers` table, no `engineer_summaries`, no engineer detail page.

*Reversed in MVP:* Full engineer module from §4.10–4.11, §6.12–6.14, §7.4–7.7.

**S-07: No audit log**

Full TOR audits changes to normatives, config, and assignments.

PoC has no audit log.

*Reversed in MVP:* Add `audit_log` table, write events on normative and config changes.

**S-08: No PDF export**

PoC exports СВОД to XLSX only. PDF export is post-MVP.

---

### 15.3 PoC Data Model

Simplified schema — replaces the full §5 schema for PoC only:

```sql
-- Org structure (unchanged from full TOR)
divisions       (id, name)
branches        (id, division_id, name)
objects         (id, branch_id, name, responsible_engineer VARCHAR, import_seq_no)

-- Device catalog — read-only seed data in PoC
...
-- Computed cache — no is_stale, no period_id (S-02)
summaries  (id, object_id,
            ...
            os_monthly_avg, ps_monthly_avg, video_monthly_avg,
            records_monthly,
            total_repairs, repair_no_travel_monthly, repair_with_travel_monthly,
...
-- Auth — single role (S-04)
users  (id, email, name, password_hash, created_at)
-- No role column, no division_id, no capacity_fte
```

Config constants (§6.11) are hardcoded in application code for PoC.
...
|---|---|
| `/login` | Login page |
| `/` | Dashboard — ИТОГО Числ by division, top 10 objects |
| `/objects` | Object list — searchable, shows ИТОГО Числ |
| `/objects/:id` | Object detail — 5 tabs: Оборудование, Записи, Ремонт, Дорога, СВОД |
| `/svod` | Full СВОД table — paginated, filterable by division/branch |
| `/svod/export` | Trigger XLSX export |

No catalog management pages, no engineer pages, no admin pages, no periods page.

---

### 15.5 PoC Acceptance Criteria

| ID | Criterion |
|---|---|
| **PAC-01** | Import of `Шаблон_нагрузки_з_v_4_00.xlsx` completes without errors, producing 2,935 object records with non-zero СВОД values. |
| **PAC-02** | For the reference object "Архив г.Брест, ул.Московская, 202Д": `itogo_chislo_with_travel = 0.032327 ±0.000001`. |
| **PAC-03** | СВОД XLSX export, when opened in Excel, matches source file ИТОГО Числ values within ±0.001 for all 2,935 rows. |
| **PAC-04** | Editing any equipment quantity in the UI and saving immediately updates the СВОД tab with recalculated values (no page refresh required). |
| **PAC-05** | СВОД table loads first 100 rows in under 3 seconds on a standard dev machine. |
| **PAC-06** | Unauthenticated requests to any route redirect to `/login`. |

---

### 15.6 PoC → MVP Migration Path

After PoC is validated, the following work items convert it to MVP. Order reflects dependency:

1. **M-01** Add planning periods (FR-12) — `periods` table, `period_id` FKs on repairs and records, period selector UI, period lock.
2. **M-02** Add RBAC — admin, editor, engineer, viewer roles; division scoping for editors.
3. **M-03** Add physical inventory layer — `object_devices` table, split equipment UI into two sections (S-01 reversal).
4. **M-04** Add device catalog management UI — add/edit device types, system contexts, normatives (S-03 reversal).
5. **M-05** Add repair type catalog management UI — add/edit repair types (S-03 reversal).
6. **M-06** Add staleness tracking and on-demand recalculation — `is_stale` flag, background worker, admin trigger (S-02 reversal).
7. **M-07** Add engineer module — `object_engineers`, `engineer_summaries`, engineer detail page, workload dashboard (S-06 reversal).
8. **M-08** Add audit log (S-07 reversal).
9. **M-09** Add PDF export (S-08).
10. **M-10** Move config constants to `app_config` table with admin UI.

Items M-01 through M-06 can run in parallel. M-07 depends on M-02 (roles must exist before engineer role is meaningful).

---

### 15.7 PoC Technology Recommendation

For speed of delivery, the PoC should use the simplest viable stack:

| Layer | PoC Recommendation | Reason |
|---|---|---|
| Backend | Python + FastAPI | Fast to write, openpyxl for import, easy calculation code |
| Database | PostgreSQL | Same as full TOR — no migration cost when moving to MVP |
| ORM | SQLAlchemy | Handles seed migrations cleanly |
| Frontend | React + Vite + TanStack Table | СВОД table needs good pagination; minimal setup |
| Auth | JWT (simple, no refresh for PoC) | Sufficient for PoC |
| XLSX import/export | openpyxl | Already used for source file analysis |
| Deployment | Docker Compose (single machine) | No infra complexity for PoC |

The database schema is designed so that PoC tables are a **strict subset** of the full TOR schema — no columns are dropped when moving to MVP, only new tables and columns are added. This protects PoC data from being discarded.

---

*End of Technical Specification — Version 2.3*
