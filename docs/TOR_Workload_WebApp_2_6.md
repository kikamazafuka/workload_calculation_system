...
### 15.8 PoC Technology Recommendation

| Layer | PoC Choice | Reason |
|---|---|---|
| Backend | Python + FastAPI | Fast to write; easy calculation code; good for PoC timeline |
| Database | PostgreSQL | Same as full TOR — zero migration cost to MVP |
| ORM | SQLAlchemy + Alembic | Handles seed migrations; Alembic migrations carry forward to MVP |
| Frontend | React + Vite + TanStack Table | СВОД needs good pagination; minimal setup |
| Auth | JWT (no refresh tokens for PoC) | Sufficient for PoC |
| XLSX export | openpyxl | Matches source file format precisely |
| Deployment | Docker Compose (single machine) | No infra complexity for PoC |

The database schema is designed so PoC tables are a **strict subset** of the full TOR schema — no columns are dropped when moving to MVP, only new tables and columns are added. PoC data survives the migration intact.
...
All application log lines must be emitted as JSON to stdout. Minimum fields on every log line:
...
Log levels used: `DEBUG` (dev only), `INFO` (normal operations), `WARN` (recoverable issues), `ERROR` (exceptions, failed operations).

No plaintext/unstructured logs permitted in any environment. Reason: structured logs are parseable by any log aggregator without reconfiguration — switching from local Docker logs to a managed log service requires zero code changes.
...
Returns HTTP 200 when the application is running and the database connection is alive:
...
Returns HTTP 503 if the database is unreachable. Used by Docker Compose healthcheck and any future load balancer.

#### Error Alerting — PoC

All unhandled exceptions and ERROR-level log events are written to a local log file with rotation (max 100MB, 7-day retention). For PoC, a daily `tail -n 50 error.log` review is the monitoring process. No automated alerting required for PoC.
...
Integrate an APM agent (OpenTelemetry + compatible backend: Sentry, Datadog, or self-hosted Grafana/Tempo). Instrument:
...
| XLSX export fails | Check `error.log` for openpyxl exception; verify template column mapping in §7.9 |
| All summaries show stale | Trigger `POST /svod/recalculate` as admin; monitor job logs |


---

*End of Technical Specification — Version 2.4*
