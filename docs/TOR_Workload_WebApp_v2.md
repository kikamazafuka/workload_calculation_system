...
- Number of objects per responsible engineer.
...
Division ──< Branch ──< Object
    ...
    (physical qty)   assignments         (Записи)
           │         (maintained qty)
           │               │
           └───────┬───────┘
...
division_id   UUID         FK → divisions.id           -- scope for editors; NULL = all
...
Captures changes to `device_system_contexts`, `device_types`, `repair_types`, `app_config`.
...
5. **СВОД** — Computed summary (read-only; shows stale indicator when `is_stale = TRUE`)
...
- All changes to `device_system_contexts`, `device_types`, `repair_types`, `app_config` written to `audit_log`.
...
- `is_stale = TRUE` is set in the **same transaction** as the data change.
...
|---|---|---|---|
| View all data | ✅ | ✅ | ✅ |
| Edit object metadata | ✅ | ✅ (own division) | ❌ |
| Edit equipment / assignments | ✅ | ✅ (own division) | ❌ |
| Edit records / repairs / travel | ✅ | ✅ (own division) | ❌ |
...
**Editor scope:** An editor's `division_id` restricts all write operations to objects in their assigned division. Enforced at the API level on every request.
...
*End of Technical Specification — Version 2.0*
